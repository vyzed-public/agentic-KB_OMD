...
## Large Media = Ephemeral Local Cache (never committed)

Large binary media — video/audio (`*.webm`, `*.mp4`, `*.mkv`, `*.mov`, `*.avi`, `*.m4a`, `*.mp3`, `*.wav`, `*.flac`) — is **never committed to git** (schema Rule 15). It is a **disposable local cache**; its durable representation is its **transcript** (committed) plus the **CTN's external-reference pointers** (see [[spec.OMD-conventions]] "Media-capture CTNs"). This keeps the repo lean and cloneable, avoids GitHub's hard limits (single file > 2 GB is rejected outright; > 100 MB blocks a regular push), and sidesteps git-LFS entirely — LFS is rejected because a file carries only one git filter, so an LFS filter would silently override `filter=git-crypt` and store media **unencrypted**.

- **Enforcement is the `.gitignore`, not agent discipline.** The repo's `.gitignore` carries **path-independent bare globs** (`*.webm`, …) so media stays ignored *wherever* it sits — top-level inbox **or** `YYYY/MM/assets/`. This is deliberate: step 11 **moves** media into the dated `assets/`, and a path-anchored ignore would let the very next `git add -A` re-sweep it into a push. Per this schema's own "rules don't enforce themselves," relying on hand-exclusion every ingest is the fragile path; the gitignore makes it deterministic.
- **Commit the transcript, not the bytes.** `.vtt`/`.srt` transcripts, `.md` notes, and images are unaffected and commit normally. The transcript is the ingestible signal and the least-lossy durable rung of the inference ladder for a video.
- **A media CTN must carry durable pointers.** Canonical source URL (re-fetchable ID, e.g. YouTube ID for `yt-dlp`) **+** remote-archive URL/ID (e.g. Google Drive) **+** transcript `[[wikilink]]` **+** the dated local path (flagged *ephemeral*). A local-only media file with no remote fallback is the one genuinely risky state — `lint` flags it (see [[spec.lint-health-check]]).
- **`git add -A` on a media drop is a footgun.** Never blanket-add a timeline drop that includes media without the gitignore in place — that is exactly how ~9 GB nearly went up in the incident that motivated this policy.
...
11. **`[direct]` File the artifact (the SEAL).** Move any staged assets (`2_using_timeline/attachments/`) → `2_using_timeline/YYYY/MM/assets/`, then move the article → `2_using_timeline/YYYY/MM/`. Use `mv` **within** the timeline (basename is preserved, so inbound basename links survive). This move is the **seal**: once filed, the note is agent-immutable — the guard blocks every further write to it. So do this **only after** stamping (step 9) and localizing (step 10) are done and verified. *(The move is **name-preserving** — basename unchanged — so inbound `[[basename]]` links survive the raw `mv`; no link-repointing tool is needed, which is why the retired `vault_move` is not missed here.)*
    - **Large media rides along but stays uncommitted.** If the drop included video/audio, `mv` it into the same `YYYY/MM/assets/` for local organization (the curator's request — attachment-locality). It is a plain filesystem move, never `git mv`: the `.gitignore` bare globs keep it ignored at the new path too, so there is **nothing to un-track and nothing to exclude by hand** (schema Rule 15 + the "Large Media = Ephemeral Local Cache" section above). Only the transcript + localized images from `assets/` are committed. **Verify before any post-ingest push** that `git status` shows no media file staged — a media file appearing as tracked means the gitignore glob is missing or was bypassed with `git add -f`; stop and fix the ignore rather than push it.
