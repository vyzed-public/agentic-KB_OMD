...
- **Malformed frontmatter field names** — a YAML key containing **parentheses, spaces, or bracket punctuation** (`(` `)` `[` `]` `{` `}` or whitespace) violates the field-name discipline in [[spec.OMD-conventions]] — legal YAML but not queryable and prone to silent propagation via clipper templates. The classic offender is `parent(s):` (rename to `parents:`, or drop it — it isn't a schema field). Scan every note's frontmatter keys; flag any that match `[()\[\]{}\s]`. (Separators `-`/`_` are fine.) Fix GWNs directly; **flag, don't edit,** a CTN/timeline note (the guard blocks editing the verbatim original) so the curator renames the key.
