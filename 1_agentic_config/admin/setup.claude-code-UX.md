...
3. **"Needs you" notification** — a persistent popup the moment Claude is blocked waiting on your decision or input (see Appendix).
4. **Terminal focus-return** — on X11, raises the terminal back to the foreground when Claude finishes or needs you (see Appendix).
...
case "$1" in
  ...
  notify)
    ...
    # Critical urgency + no timeout => the popup persists until you act on it.
    notify-send -u critical -t 0 -i dialog-question -h int:suppress-sound:1 \
...
**What this does:**
- **`start`** (you submit a prompt): writes `busy` to `~/.claude/processing_state` (statusline reads this); on X11, records this terminal's window ID for focus-return.
- **`stop`** (Claude finishes): writes `idle`, fires a silent "✓ Done — ready for input" popup (5s), and returns focus to the terminal.
- **`notify`** (Claude is blocked waiting on you — a tool-permission decision or idle input): fires a **persistent** "⏳ needs you" popup (critical urgency, no timeout) and returns focus to the terminal.
- Focus-return is X11-only and a silent no-op elsewhere — see the Appendix.
...
**Important:** Replace `YOUR_USERNAME` with the actual username (run `whoami` to confirm). Use the full absolute path — do not use `~` or `$HOME` in the command strings.
...
1. **Focus-return** — when Claude finishes *or* blocks waiting on you, the terminal it runs in is raised back to the foreground, so you don't have to hunt for it. It records the terminal's window ID on `start` (`xdotool getactivewindow`) and re-activates it on `stop`/`notify` (`xdotool windowactivate`).
2. **"Needs you" notification** — a *separate, persistent* popup (critical urgency, no timeout) that fires the moment Claude is blocked on a decision — a **tool-permission prompt**, or idle input. This closes a real gap: a permission prompt pauses Claude *mid-turn*, so the `Stop` hook never fires for it — without this you'd get **no signal at all**. It's wired via Claude Code's **`Notification` hook** → `title-hook.sh notify`.
...
3. Merge the `statusLine` + `UserPromptSubmit`/`Stop`/`Notification` hooks into `~/.claude/settings.json` (Step 3), replacing `YOUR_USERNAME` (run `whoami`).

**Verify:** trigger a tool-permission prompt (or let the prompt idle ~60s) → a persistent "⏳ needs you" popup appears and the terminal jumps to the foreground. On completion → the transient "✓ Done" popup + focus-return.
