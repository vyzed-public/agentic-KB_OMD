...
15. **Large binary media is never committed — it is an ephemeral local cache.** Video/audio files (`*.webm`, `*.mp4`, `*.mkv`, `*.mov`, `*.avi`, `*.m4a`, `*.mp3`, `*.wav`, `*.flac`) never enter git; a **path-independent `.gitignore`** keeps them out *wherever* they sit — top-level inbox **or** `2_using_timeline/YYYY/MM/assets/`. What is durable gets committed in their place: the **transcript** (`.vtt`/`.srt`) — the ingestible signal and a rung of the inference ladder — and the **CTN's reference pointers** (canonical source URL + remote-archive URL/ID + transcript `[[wikilink]]` + the dated local path, flagged *ephemeral*). At ingest, still `mv` media into its dated `assets/` for local organization; the gitignore keeps it uncommitted **automatically** — never hand-exclude, and never `git add -A` a media file into a push. This separates **content durability** (transcript-in-git, encrypted + versioned) from **byte durability** (external archive + canonical source). git-LFS is **rejected**: a file carries only one git filter, so an LFS filter would silently override `filter=git-crypt` and store media **unencrypted**. Full procedure: [[spec.file-ingestion]]; CTN template: [[spec.OMD-conventions]]; lint checks: [[spec.lint-health-check]].
